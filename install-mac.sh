#!/bin/bash
# ============================================================
#  ShimiStudio Worker Installer — macOS (Apple Silicon / Intel)
#  גרסה קנונית — שימי פיתוח, 09.09.2026
#  הרצה: curl -fsSL <כתובת> | bash
#  או:   bash install-mac.sh
# ============================================================
set -e
...
echo ""
echo "========================================"
echo "  ShimiStudio Worker - Installer (Mac)"
echo "========================================"
echo ""
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "  Python3 לא נמצא — מפעיל התקנת Command Line Tools..."
  xcode-select --install 2>/dev/null || true
  echo "  המתן לסיום ההתקנה בחלון שנפתח, ואז הרץ שוב את הסקריפט."
  exit 1
fi
...
if [ -f "ComfyUI/main.py" ]; then
  ...
  git clone --depth 1 https://github.com/comfyanonymous/ComfyUI.git 2>/dev/null || {
    err "הורדת ComfyUI נכשלה — בדוק חיבור לאינטרנט"; exit 1; }
  ok "ComfyUI הותקן"
fi
COMFYUI_DIR="$DIR/ComfyUI"

# ── 3.5. Custom nodes ──
step 3.5 "Custom nodes (IP-Adapter, ReActor, AnimateDiff)..."
NODES_DIR="$COMFYUI_DIR/custom_nodes"
mkdir -p "$NODES_DIR"
clone_node() {
  local dir="$1" url="$2"
  if [ -d "$NODES_DIR/$dir" ]; then ok "$dir קיים"; return 0; fi
  git clone --depth 1 "$url" "$NODES_DIR/$dir" 2>/dev/null && ok "$dir הותקן" || err "$dir נכשל (לא חוסם)"
}
...
# ── 4. סביבה + תלויות ──
step 4 "מכין venv ומתקין תלויות (PyTorch + MPS)..."
if [ ! -x "venv/bin/python" ]; then
  "$PY" -m venv venv
fi
VP="$DIR/venv/bin/python"
...
if [ ! -d "sd-scripts" ]; then
  git clone --depth 1 https://github.com/kohya-ss/sd-scripts.git 2>/dev/null || err "sd-scripts נכשל (training ייפול ל-IP-Adapter)"
fi
if [ -f "sd-scripts/requirements.txt" ]; then
  grep -v -e '^#' -e '^-e' -e '^\.' sd-scripts/requirements.txt > /tmp/sd-reqs.txt 2>/dev/null || true
  "$VP" -m pip install -r /tmp/sd-reqs.txt --quiet 2>/dev/null || err "חלק מתלויות ה-training נכשלו (לא חוסם)"
  "$VP" -m pip install accelerate transformers --quiet 2>/dev/null || true
  ok "sd-scripts מוכן"
fi

# ── 4.5. מודלים ──
step 4.5 "מודלים (בפעם הראשונה: ~11GB, יתכנו כמה דקות)..."
dl() {
  local dest="$1" url="$2" label="$3"
  if [ -f "$dest" ]; then ok "$label קיים"; return 0; fi
  echo "  מוריד $label..."
  curl -L --fail --retry 3 --progress-bar -o "$dest" "$url" && ok "$label הורד" || err "$label נכשל (לא חוסם)"
}
CKPT="$COMFYUI_DIR/models/checkpoints"
IPAD="$COMFYUI_DIR/models/ipadapter"
CLIPV="$COMFYUI_DIR/models/clip_vision"
MM="$COMFYUI_DIR/models/animatediff_models"
mkdir -p "$CKPT" "$IPAD" "$CLIPV" "$MM"
dl "$CKPT/v1-5-pruned-emaonly.safetensors" "https://huggingface.co/stable-diffusion-v1-5/stable-diffusion-v1-5/resolve/main/v1-5-pruned-emaonly.safetensors" "Stable Diffusion 1.5 (~4GB)"
dl "$IPAD/ip-adapter-plus_sd15.safetensors" "https://huggingface.co/h94/IP-Adapter/resolve/main/models/ip-adapter-plus_sd15.safetensors" "IP-Adapter Plus SD1.5 (~1.5GB)"
...
TOKEN=$("$PY" -c "import secrets; print(secrets.token_hex(12))")
...
read -n 1
EOF
chmod +x "$DIR/start_worker.command"
cp "$DIR/start_worker.command" "$HOME/Desktop/ShimiStudioWorker.command" 2>/dev/null && chmod +x "$HOME/Desktop/ShimiStudioWorker.command" && ok "קיצור-דרך בשולחן העבודה" || ok "סקריפט הפעלה ב-$DIR"
...
echo "  מכשיר: Mac | Token: ${TOKEN:0:8}..."
echo "  להפעלה בעתיד: לחיצה כפולה על ShimiStudioWorker.command בשולחן העבודה"
echo "========================================"
echo ""
exec "$VP" "$DIR/worker.py"
